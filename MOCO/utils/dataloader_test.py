...
def load_from_numpy(file, shuffle_ratio):
    ...
    batch_label = data['y']
    #--------------------
    #batch_label = np.array(list(len(data['y'])*"1"),dtype=float)
    #--------------------
...
def split_and_load(file, shuffle_ratio, split_now, split_savedir = "./Fugue/Data/example_splitfile/"):
    ...
    if split_now == True:
        # data = np.load("/mnt/hpc/home/shilei/RongfangNie/project/Nierongfang/Moco/code/Fugue/data/splatter_simulation.npz")
        data = np.load(file,allow_pickle=True)
        expr = data['x']
        #batch_label = data['y']
        batch_label = np.array(list(len(data['y'])*"1"))
